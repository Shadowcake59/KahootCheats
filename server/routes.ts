...
class KahootService {
  ...
  private setupKahootClientEvents(client: Kahoot, gamePin: string, ws: WebSocket) {
    ...
    client.on("questionStart", async (question: any) => {
      log(`Question started: ${question.index + 1}/${question.quiz.questionCount}`, "kahoot");
    ...
    client.on("questionEnd", async (questionResult: any) => {
      ...
      if (game) {
        ...
        const session = await storage.getGameSessionByPin(gamePin);
        if (session) {
          const question = await storage.getQuestionByIndex(session.id, questionResult.questionIndex);
          if (question) {
            ...
            await storage.updateQuestion(question.id, {
              ...
              correctAnswer: correctIndices.length > 0 ? updatedAnswers[correctIndices[0]] : null
            });
          }
        }
  ...
  async getGameState(gamePin: string): Promise<any> {
    try {
      ...
      const currentQuestion = await this.getCurrentQuestion(gamePin);
      const previousQuestion = game.currentQuestionIndex > 0 
        ? await storage.getQuestionByIndex(session.id, game.currentQuestionIndex - 1)
        : null;
  ...
  async selectAnswer(gamePin: string, answerIndex: number, ws: WebSocket): Promise<boolean> {
    try {
      ...
      log(`Sending answer ${answerIndex} for question ${game.currentQuestionIndex + 1}`, "kahoot");
  ...
  async disconnectFromGame(gamePin: string, ws: WebSocket): Promise<boolean> {
    try {
      ...
      log(`Error disconnecting from game: ${(error as Error).message}`, "kahoot");
      return false;
    }
  }
}
        ...
        switch (validatedData.type) {
          case 'join':
            if (validatedData.gamePin) {
              const success = await kahootService.joinGame(validatedData.gamePin, ws);
              if (success) {
          ...
          case 'selectAnswer':
            ...
            if (clientInfo && clientInfo.gamePin && validatedData.answer?.index !== undefined) {
